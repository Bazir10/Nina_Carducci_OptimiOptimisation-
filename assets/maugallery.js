    ...
    $.fn.mauGallery.methods = {
      ...
      wrapItemInColumn(element, columns) {
        if (columns.constructor === Number) {
          ...
          console.error(`Columns should be defined as numbers or objects. Received ${typeof columns}`);
        }
      },
      ...
      prevImage(lightboxId) {
        ...
        let $activeImage = $(`.gallery-item[src="${activeImageSrc}"]`);
        let activeTag = $(".active-tag").data("images-toggle");
        let $prevImage = $activeImage;
        do {
          $prevImage = $prevImage.closest('.item-column').prev().find('.gallery-item');
        } while (activeTag !== "all" && $prevImage.data("gallery-tag") !== activeTag && $prevImage.length !== 0);
        if ($prevImage.length === 0) {
          $prevImage = (activeTag === "all") ? $('.gallery-item').last() : $(`.gallery-item[data-gallery-tag="${activeTag}"]`).last();
        }
      ...
      nextImage(lightboxId) {
        ...
        let $activeImage = $(`.gallery-item[src="${activeImageSrc}"]`);
        let activeTag = $(".active-tag").data("images-toggle");
        let $nextImage = $activeImage;
        do {
          $nextImage = $nextImage.closest('.item-column').next().find('.gallery-item');
        } while (activeTag !== "all" && $nextImage.data("gallery-tag") !== activeTag && $nextImage.length !== 0);
        if ($nextImage.length === 0) {
          $nextImage = (activeTag === "all") ? $('.gallery-item').first() : $(`.gallery-item[data-gallery-tag="${activeTag}"]`).first();
        }
        $(`#${lightboxId} .lightboxImage`).attr("src", $nextImage.attr("src"));
      },
      createLightBox(gallery, lightboxId, navigation) {
        lightboxId = lightboxId ? lightboxId : "galleryLightbox";

...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage:function(lightboxId){let activeImageSrc=$(`#${lightboxId} .lightboxImage`).attr("src");let $activeImage=$(`.gallery-item[src="${activeImageSrc}"]`);let activeTag=$(".active-tag").data("images-toggle");let $prevImage=$activeImage;do{$prevImage=$prevImage.closest('.item-column').prev().find('.gallery-item')}while(activeTag!=="all"&&$prevImage.data("gallery-tag")!==activeTag&&$prevImage.length!==0);if($prevImage.length===0){$prevImage=(activeTag==="all")?$('.gallery-item').last():$(`.gallery-item[data-gallery-tag="${activeTag}"]`).last()}
$(`#${lightboxId} .lightboxImage`).attr("src",$prevImage.attr("src"))},nextImage:function(lightboxId){let activeImageSrc=$(`#${lightboxId} .lightboxImage`).attr("src");let $activeImage=$(`.gallery-item[src="${activeImageSrc}"]`);let activeTag=$(".active-tag").data("images-toggle");let $nextImage=$activeImage;do{$nextImage=$nextImage.closest('.item-column').next().find('.gallery-item')}while(activeTag!=="all"&&$nextImage.data("gallery-tag")!==activeTag&&$nextImage.length!==0);if($nextImage.length===0){$nextImage=(activeTag==="all")?$('.gallery-item').first():$(`.gallery-item[data-gallery-tag="${activeTag}"]`).first()}
$(`#${lightboxId} .lightboxImage`).attr("src",$nextImage.attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
